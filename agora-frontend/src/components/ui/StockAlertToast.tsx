...
export default function Toast() {
  ...
  return (
    ...
    >
      {alerts.map((alert) => (
        <ToastItem key={alert.id} alert={alert} onDismiss={() => dismissAlert(alert.id)} />
      ))}
    </div>
  )
}
...
interface ToastItemProps {
  alert: { id: string; productName: string; quantity: number; threshold: number; timestamp: number }
  onDismiss: () => void
}

function ToastItem({ alert, onDismiss }: ToastItemProps) {
  // each toast gets its own independent 6s timer, keyed by its own id —
  // adding a new alert no longer resets/steals the timer from an existing one
  useEffect(() => {
